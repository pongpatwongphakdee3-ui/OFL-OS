...
description: สร้าง Instagram carousel/slide content ครบทุก slide สำหรับช่อง AI. รองรับ 2 mode: Generate (สร้างจากศูนย์) และ Convert (แปลง script ที่มีอยู่แล้วเป็น carousel). Trigger: /carousel, /slide, /ig-carousel หรือ intent เช่น "สร้าง carousel เรื่อง...", "แปลง script นี้เป็น carousel"
---
...
หรือมี intent ที่ชัดเช่น "สร้าง carousel เรื่อง...", "ทำ slide สำหรับ IG เรื่อง...", "แปลง script นี้เป็น carousel", "ช่วย generate slides"
...
ก่อนเริ่ม ดูว่า user อยู่ใน mode ไหน:

| สัญญาณ | Mode |
|--------|------|
| User paste script / บอกว่า "มี script อยู่แล้ว" / "แปลงจาก script นี้" | **Convert** → ไป STEP 1B |
| User บอกหัวข้อหรือ idea แต่ยังไม่มี script | **Generate** → ไป STEP 1A |
| ไม่ชัดว่า mode ไหน | ถามแค่: "มี script อยู่แล้วไหมครับ หรืออยากให้ช่วยสร้างจากศูนย์?" |

---

## STEP 1A — Generate Mode (สร้างจากศูนย์)

ถามคำถามต่อไปนี้พร้อมกันทีเดียว เว้นแต่ user บอกมาครบแล้วใน prompt:
...
## STEP 1B — Convert Mode (มี script อยู่แล้ว)

**ไม่ต้องถาม 7 คำถาม** — อ่าน script แล้ว extract เอง

หลัง user paste script มาแล้ว ถามแค่ 3 ข้อที่ script ไม่บอก:

```
ได้เลยครับ อ่าน script แล้ว

ขอถามแค่ 3 ข้อ:
1. CTA เป้าหมาย: save / follow / DM / comment keyword? (ถ้าไม่มีใน script)
2. จำนวน slides: ต้องการกี่ slides? (default: 8)
3. Seed อะไรไหม: มีอะไรอยากแทรกแบบ soft ไหม? (ถ้าไม่มีบอก "ไม่มี")
```

**Conversion Logic — ทำตามลำดับนี้:**

**1. วิเคราะห์ script ก่อน generate:**
- Main claim หรือ contrarian belief ที่ script ต้องการสื่อ → จะเป็น Slide 1 Hook
- Problem ที่ script พูดถึง → Slide 2
- Reframe หรือ new perspective → Slide 3
- Key points / breakdown จาก body ของ script → Slide 4–6
- Example หรือ personal story ใน script → Slide 7
- Closing message หรือ takeaway → Slide 8

**2. Compress ให้เหมาะกับ slide format:**
- Script มักยาวเกินสำหรับ slide — ตัดให้เหลือ core idea ต่อ slide
- เลือกเฉพาะสิ่งที่ทำให้คน save หรือ swipe — ไม่ต้องเอาทุกประโยค
- ประโยคที่ดีใน script → เก็บไว้ใกล้เคียงเดิม ไม่ต้อง rewrite ทั้งหมด

**3. ถ้า script ขาด element ใด:**
- ขาด hook ที่แรงพอ → สร้าง hook ใหม่จาก main claim ของ script
- ขาด example → ใช้ตัวอย่างที่ implicit อยู่ใน script หรือถาม user เพิ่ม
- ขาด reframe → หา contrarian angle จาก content ที่มี
- ไม่ต้อง invent ข้อเท็จจริงใหม่ที่ไม่มีใน script

**4. Apply voice rules เหมือน Generate mode ทั้งหมด**

หลังจากแปลงเสร็จ บอก user ด้วยว่า:
- ตัด/ย่อส่วนไหนออก และทำไม
- เพิ่มส่วนไหนเข้าไป เพราะ script ขาด
...
- ถ้ามีประสบการณ์ตรง (Generate mode) หรือ personal story ใน script (Convert mode) → ใช้ "ผมเคย..." เปิด
- ถ้าไม่มี → ใช้ concrete scenario ที่ audience เจอได้จริง
- 1 ตัวอย่างที่ดีดีกว่า 3 ตัวอย่างที่กลาง
- ถ้ามี seed → วางที่นี่แบบ soft: "ถ้าอยากเจาะลึกกว่านี้..."
...
ใช้ข้อมูลจาก STEP 1A ข้อ 7 (Generate mode) แล้ว apply logic นี้:
...
- Convert notes: [ส่วนที่ตัดออก / เพิ่มเข้ามา — เฉพาะ Convert mode]
```
